## Description
The application is vulnerable to Insecure Direct Object References (IDOR). The endpoint at {URL} exposes a reference to an internal implementation object (such as a database key or filename) without proper authorization checks. By modifying this value, an attacker can access or manipulate data belonging to other users.
