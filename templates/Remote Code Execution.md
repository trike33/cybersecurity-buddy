## Description
The application is vulnerable to Remote Code Execution (RCE) at {URL}. Due to improper sanitization of user input passed to system shells or code evaluation functions (like `eval()`, `exec()`, or `system()`), an attacker can execute arbitrary system commands on the server.
