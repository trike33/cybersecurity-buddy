## Description
The application does not enforce protection against Cross-Site Request Forgery (CSRF) on state-changing requests at {URL}. This allows an attacker to force an authenticated user's browser to send a forged request to the vulnerable application without the user's consent or knowledge.
